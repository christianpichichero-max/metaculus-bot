date: 2026-09-07T17:20:19Z
resolved: 80 (was 78)

204 log lines | 204 unique questions
fetching 204 posts...
first pass: 18 fetch failures — cooling down 30s and retrying those...
WARNING: 1 of 204 posts could not be fetched (rate limit/error). Report is INCOMPLETE — resolved.jsonl NOT overwritten; re-run.

RESOLVED binary: 80 | Brier 0.2248 (lower=better, 0.25=coinflip) | LogLoss 0.6648

Reliability (predicted bucket vs actual yes-rate):
   0- 10% | n= 18 | actual 22%
  10- 20% | n= 21 | actual 19%
  20- 30% | n=  9 | actual 33%
  30- 40% | n=  6 | actual 33%
  40- 50% | n=  7 | actual 57%
  50- 60% | n=  6 | actual 67%
  60- 70% | n=  4 | actual 25%
  70- 80% | n=  7 | actual 43%
  80- 90% | n=  1 | actual 0%
  90-100% | n=  1 | actual 100%

RESOLVED multiple-choice: 16 | avg ln p(winner) -1.533 | multiclass Brier 0.7470

RESOLVED numeric/discrete/date: 55 | P10-P90 coverage 85% (target ~80%) | P2.5-P97.5 coverage 93% (target ~95%) | beyond-declared-tails: 2

OFFICIAL scores captured on 151 question(s): SPOT PEER total = -662.43 (prize share ∝ max(0, total)²)

SUPERVISOR SHADOW A/B (30 resolved fired-questions):
  geo-odds (submitted)        Brier: 0.2571
  ship-policy (high-conf only) Brier: 0.2948  ← geo-odds better/tied
  diagnostic (always-trust)    Brier: 0.2688
  high-confidence overrides (4): geo-odds 0.3677 vs supervisor 0.6507
  Gate: flip use_supervisor=True only if ship-policy wins on ≥30 fired resolutions.

BLF SHADOW A/B (41 resolved):
  submitted (ensemble geo-odds) Brier: 0.2199
  belief-loop (shadow)          Brier: 0.2704  ← ensemble better/tied
  Gate: promote the BLF to the forecaster seat only if it wins on ≥40 resolutions.
